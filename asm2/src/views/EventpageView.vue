...
const loadAsyncData = () => {
    if (token.value) {
        ...
        fetch(`/api/event/?page=${route.params.page}`, {
            headers: {
                Authorization: `Bearer ${token.value}`
            }
        ...
        fetch(`/api/event/?page=${route.params.page}`)//加返page ge 野 lab6 ge 野
            .then((response) => response.json())
            .then((result) => {
...
<template>
        ...
        <div class="card col-md-4" v-for="ev in events" :key=ev.id>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col" v-if="role == 'admin'">
                        <RouterLink :to="`/event/edit/${ev._id}`" class=" btn btn-primary btn-lg float-end">Edit
                        </RouterLink>
                    </div>
    ...
    <nav aria-label="Page navigation example">
        ...
        <ul class="pagination">
            <li class="page-item" v-for='ea in pa' :key="ea"><a class="page-link" :href="`/event/page/${ea}`"
                    :class="{ active: route.params.page == ea }">{{ ea }}</a></li>
